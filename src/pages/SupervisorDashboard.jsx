...
import { Users, BarChart, Settings, X, Save } from 'lucide-react';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { error } = await supabase
        .from('clients')
        .insert([{ ...formData, asesor_id: user.id }]);
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="text-muted">Gestión de equipo, ventas y actualización de estados.</p>
        </div>
        <button className="btn btn-primary" onClick={() => setShowModal(true)}>
          <Users size={18} /> Nuevo Registro
        </button>
      </div>
      ...
      <div className="kpi-grid">
        <div className="kpi-card">
          ...
          <div className="kpi-label">Ventas Totales (Equipo)</div>
        </div>
        <div className="kpi-card">
        ...
        <div className="kpi-card">
          <div className="kpi-icon green"><Settings size={24} /></div>
          <div className="kpi-value">{activos}</div>
          <div className="kpi-label">Instalaciones Exitosas</div>
        </div>
      </div>

      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <div className="card-header" style={{ padding: '1.5rem', marginBottom: 0, borderBottom: '1px solid var(--border)' }}>
          <h3 style={{ margin: 0, fontSize: '1.125rem' }}>Gestión de Clientes</h3>
        </div>
        ...
        <div className="table-responsive">
          <table className="table">
            ...
            <tbody>
              ...
              {clients.length === 0 && (
                <tr><td colSpan="5" style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>No hay clientes registrados en tu equipo.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {showModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              <h3 style={{ margin: 0 }}>Registrar Nueva Venta / Ruta</h3>
            ...
            <div className="modal-body">
              <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                ...
                <div className="form-group">
                  ...
                  <input type="text" name="route_name" className="form-input" placeholder="Ej: Zona Norte, Barrio Centro..." required value={formData.route_name} onChange={handleChange} />
                </div>
                <div className="form-group">
                ...
                <div style={{ gridColumn: '1 / -1', display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1rem' }}>
                  ...
                  <button type="submit" className="btn btn-primary"><Save size={18} /> Guardar Registro</button>
                </div>
              </form>
            </div>
          </div>
